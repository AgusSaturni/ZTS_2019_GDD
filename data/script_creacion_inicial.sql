...
AS BEGIN
if(@nombre_rol_nuevo = @nombre_rol_viejo)
throw 50004,'el nombre nuevo del rol que desea actualizar coincide con el nombre actual',1
if not exists(select 1 from ZTS_DB.ROLES where Rol_Id = @nombre_rol_nuevo)
	begin
	...
	throw 50003,'Ya existe otro rol con ese nombre. Elija otro',1
END
GO
